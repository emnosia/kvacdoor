...
?><div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="mb-0 font-size-18"><i class="fa fa-globe"></i> <?= $title ?></h4>
...
<div class="row">
    <div class="col-lg-5">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title"><i class="fa fa-cog" aria-hidden="true"></i> Attack Hub L7</h4>

                <form>
                    <div class="form-group">
                        <label for="hostInput">Host</label>
                        <input type="text" class="form-control" id="hostInput" aria-describedby="hostHelp" placeholder="https://kvacdoor.cz">
                        <small id="hostHelp" class="form-text text-muted">Enter a website address</small>
                    </div>

                    <div class="form-group">
                        <label for="exampleFormControlSelect1">Method</label>
                        <select class="form-control" id="exampleFormControlSelect1">
                            <?php foreach($supportCategories as $key => $category): ?>
                                <optgroup label="<?= $category['name'] ?>" style="color:<?= $category['color'] ?>">
                                    <?php foreach($methods as $method): ?>
                                        <?php if ($method['type'] != $key) continue; ?>
                                        <?php $isDisabled = ($method['active'] == 0 || $method['plan'] > $AUTHUSER['network_plan']) ? 'disabled="disabled" ' : ""; ?>
                                        <option <?= $isDisabled ?>name="<?= $method['name'] ?>"><?= $method['name'] ?></option>
                                    <?php endforeach; ?>
                                </optgroup>
                            <?php endforeach; ?>
                        </select>
...
function stopAttack(id) {
    ...
    .then(data => {
        ...
        // Swal.fire({
        //     'title': 'Success',
        //     'text': 'Attack stopped successfully!',
        //     'type': 'success',
        //     'timer': 0x3e8
        // });
...
function getServer()
{
    $.ajax({
        ...
        $.each(data, function (index, attack) {
            index++;
            console.log(attack);
            $('#downlist').append('<tr><th scope="row">' + index + '</th><td>' + escapeHtml(attack.host) + '</td><td>' + escapeHtml(attack.method) + '</td><td class="time-remaining">' + escapeHtml(attack.remaining) + '</td><td><span class="badge badge-success">Ongoing</span></td><td><button class="btn btn-danger btn-sm" onclick="stopAttack('+ attack.id +')"><i class="fa fa-power-off" aria-hidden="true"></i> Stop</button></td></tr>');
        });
    });
}
